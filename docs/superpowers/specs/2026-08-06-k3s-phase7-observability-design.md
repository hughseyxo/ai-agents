...
All 8 tasks of the implementation plan executed. `kube-prometheus-stack` runs in the `monitoring` namespace (Prometheus/Alertmanager/Grafana/node-exporter/kube-state-metrics), Grafana reachable at `NodePort 30802` via Tailscale MagicDNS, all 4 `PrometheusRule` groups evaluate cleanly, and `security-audit` check #17 is live on the weekly cron.

**Real bugs found and fixed during Task 6's live-cluster apply (three rounds, not one):**

1. **NetworkPolicy egress was under-scoped in a non-obvious way.** A plain `ports: [443]` rule with no `to:` (meaning "unrestricted destination" per the NetworkPolicy spec) did NOT actually cover the kube-apiserver — kube-router (k3s's embedded NetworkPolicy controller) rejected that traffic with "connection refused", breaking the chart's admission-webhook certgen job. Root cause: the `kubernetes` Service (ClusterIP `10.43.0.1`) has no pod-backed Endpoints — it points directly at the control-plane node's real IP, which an unrestricted-destination rule didn't reliably cover under kube-router. After fixing that, Prometheus itself still couldn't reach several real scrape targets (node-exporter and the apiserver are hostNetwork-bound; coredns/Grafana/Alertmanager use ports never enumerated in the original narrow allowlist). Rather than chase each port individually — which doesn't scale for a namespace whose entire job is scraping arbitrary metrics endpoints — the policy was redesigned around **destination** scope (pod CIDR `10.42.0.0/24` + Tailscale's global CGNAT range `100.64.0.0/10`, not a host-specific IP) rather than a port allowlist. One near-miss caught in the process: an early fix draft hardcoded this host's literal Tailscale IP into a tracked manifest, violating CLAUDE.md's placeholder convention — caught before committing and replaced with the non-host-specific CGNAT range (the same pattern already used safely in the abandoned Split DNS work's tracked Traefik middleware).
2. **The `DirectoryOrCreate` textfile-collector hostPath was created `root:root 0755`**, blocking the `agent-health` CronJob's uid-1001 writes (whichever pod — this CronJob or node-exporter's DaemonSet — happens to run first on a node creates it). Fixed with an `initContainer` that chowns it to `1001:1001`; required explicitly re-adding `CHOWN` to the container's capability set, since `capabilities: drop: ["ALL"]` strips `CAP_CHOWN` even from uid 0 — root alone isn't sufficient once a container's full capability set is dropped.
3. **The metric file itself was unreadable by node-exporter even after the ownership fix.** `tempfile.mkstemp()` creates files `0600` (owner-only) by default; node-exporter scrapes the textfile collector as a different uid and silently found nothing there (no error — the target file just didn't exist as far as it could see). Fixed with an explicit `os.chmod(tmp_path, 0o644)` before the atomic rename.

**Unrelated pre-existing bug that blocked CI mid-phase:** `agents/telegram_client.py` — imported by `agent_health.py` (already on `main`) and `agents/job_alert.py` (separate in-progress work) — had never actually been committed; it existed only as an untracked file left over from another session, per this project's dual-agent (Claude/Antigravity) workflow. CI's `test` job failed collection on `ModuleNotFoundError` the first time Phase 7's code changes were pushed. Fixed by committing the file (credential-free, reads `CONCIERGE_BOT_TOKEN`/`TELEGRAM_USER_ID` from env) — unblocked the `ai-agents-runner` image rebuild Phase 7's own `agent_health.py` change needed to actually ship.

**Design doc correction found via check #17 itself, not assumed away:** the design doc's own "Current state" section (above) claimed every workload namespace already had a default-deny-egress `NetworkPolicy`. Running the new check against the real cluster surfaced that this was wrong — `wedding-ui` (Phase 3) never actually got one. This is a genuine pre-existing gap, not introduced by this phase, and left unfixed here (out of Phase 7's scope — Phase 3 is already marked complete) — flagged to the human partner rather than silently patched or silently ignored.

**One RBAC finding accepted as expected, not fixed:** check #17's RBAC audit also flags `helm-kube-system-kube-prometheus-stack`'s `ServiceAccount` holding `cluster-admin` — this is k3s's own internal `helm-controller` mechanism (every `HelmChart` CRD install gets a per-release SA with cluster-admin to install arbitrary charts), not a user-introduced risk. The check's `expected_cluster_admin_subjects` allowlist wasn't extended to cover it, since doing so is a judgment call about the check's own design rather than something this phase's plan called for — left as an honestly-reported finding rather than silently suppressed.

**Verification evidence (Task 6-7):** all 9 Prometheus scrape targets (`apiserver`, `coredns`, `kube-state-metrics`, `kubelet`, `node-exporter`, Grafana, Alertmanager, the Operator, Prometheus itself) reported `up`; all 7 rules across the 4 `PrometheusRule` groups evaluated `ok`; the `agent_health_last_success_timestamp` metric was confirmed flowing end-to-end (file write → node-exporter's own `/metrics` → Prometheus's `/api/v1/query`) after a manually-triggered `agent-health` run; positive-reachability regression checks (`docker ps` count, default-route HTTP code, both live workloads' `/healthz`) held identical to baseline at every checkpoint. Rollback drill actually executed: `kubectl delete -k k8s/base/monitoring` left the seedbox stack and both live workloads completely unaffected, then re-applied cleanly (Grafana `200` within ~90s).
