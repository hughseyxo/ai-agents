...
Python handles: lifecycle, SQLite state, plant watering logic, dedup.
Claude CLI (with MCP access) handles: calendar/todoist fetching, formatting, email sending.
"""
...
class DailyBriefingAgent(BaseAgent):
    ...
    def _build_prompt(self, today: str, plant_data: dict) -> str:
        ...
        if plants:
            plant_lines = []
            for p in plants:
                plant_lines.append(f"- {p['next_water_date']} — Water {p['name']} (in {p['days_until']} days)")
            plant_section = "\n".join(plant_lines)
        else:
