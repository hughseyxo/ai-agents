...
- **Status:** Phases 1–4 complete (on branch `obsidian-vault-backend`). Phase 5 (CouchDB + livesync-bridge infra) pending — work goes into `~/git/yopflix/seedbox/docker-compose.yaml`. Spec: `docs/superpowers/specs/2026-06-19-obsidian-vault-backend-design.md`.
...
- **Phase 5 remaining:** add `couchdb` + `livesync-bridge` services to `~/git/yopflix/seedbox/docker-compose.yaml`. CouchDB bind: `100.96.86.73:5984` (never `0.0.0.0`). Three vault mappings: `docs/` → `/`, `~/.claude/.../memory/` → `/_memory/`, `CLAUDE.md` → `/_project/`.
