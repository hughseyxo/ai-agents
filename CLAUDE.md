...
- `<TAILSCALE_IP>` below is this host's Tailscale IP, kept out of tracked files — see `TAILSCALE_IP` in `.env` (gitignored), or run `tailscale ip -4` on the host.
- **What:** server-hosted Obsidian vault (Tailscale-only, `<TAILSCALE_IP>:5984`) over `docs/` + Claude memory folder + `CLAUDE.md`, synced via **CouchDB + livesync-bridge** (Docker, lives in yopflix seedbox stack). Agents keep writing plain `.md`; bridge mirrors disk ⇄ CouchDB.
...
- **Dashboards & MOCs:** `docs/_dashboards/` — Home MOC, Plant Health, Librarian Intelligence, Memory. Plant index is `docs/plants/_index.md`. Dataview queries render on Obsidian clients only.
...
- Execution model: Python handles lifecycle, state (SQLite), retry, dedup, and deterministic logic (e.g. plant watering, weather, RSS fetching). LLM CLI (with MCP tools) handles data synthesis and formatting. **Email is sent directly via `agents/gmail_client.py`** (no LLM/MCP transport) — the LLM is reserved for content synthesis (translation, scoring, status text), not sending.
...
- Each agent declares its own cron schedule via `cron_entries()`; `python3 -m agents install-cron` writes ALL crontab entries from every agent's `cron_entries()` (including librarian's non-default `--mode` entries) into the managed block — it is the single source of truth for crontab, not a partial list
...
- Data lives in `data/agents.db`, row-per-plant in the dedicated `plants` table, managed by `PlantStore` (`agents/plant_model.py`) — tools, agents, and the daily briefing all read/write through it. The old single-blob state key (`agent="daily-briefing", key="plants"`) is migrated lazily on first read and left in place as a legacy/rollback copy only; it is NOT kept in sync after migration, so nothing should read/write it directly outside `PlantStore`. CLI tool is `plant.sh` (add/list/remove/--outdoor)
...
- **Port:** 8765, bound to `0.0.0.0` (Tailscale: `<TAILSCALE_IP>:8765`).
...
- **Batch photo upload:** dashboard "Batch Upload" FAB → `POST /api/plants/batch-photos` (multi-file) creates a job processed by `agents/photo_batch.py` (`asyncio.create_task`, off the event loop via `asyncio.to_thread` — the underlying `claude` CLI calls are blocking subprocesses). Per photo: a combined identify+preliminary-assess Opus call (`claude_backend.identify_and_assess`) against the known-plant list; on a confident match, a second "trend-refine" call folds in the plant's last 3 stored photos (`claude_backend.assess_image(..., extra_image_paths=...)`) for wilting-over-time language. Unmatched photos surface a manual-assign dropdown (`POST .../items/{index}/assign`). Frontend polls `GET /api/plants/batch-photos/{job_id}` every ~3s.
  - **Rate-limit pause/resume:** `claude_backend.detect_usage_limit()` pattern-matches CLI stdout/stderr for usage-limit signals (vs. a one-off photo failure). On a hit, the job pauses, persists state, and pings every 5 min (`PING_INTERVAL_SECONDS`) up to a 2h cap (`MAX_PAUSE_SECONDS`) before giving up (`status=failed`). Job state (`photo_batch_jobs` table) and a job-scoped temp dir (`data/plant-photo-batches/<job_id>/`) persist across a `plant_ui` service restart — resumed automatically via the FastAPI `lifespan` startup hook (`photo_batch.resume_active_jobs()`).
  - **Photo history:** every assessment (single-photo endpoint too, not just batch) saves the photo permanently to `data/plant-photos/<slug>/` and a `plant_photos` DB row (last 10 kept, oldest pruned) — this is what feeds the trend-refine call and the plant detail view's photo-timeline strip. Neither `data/plant-photos/` nor `data/plant-photo-batches/` sync to Obsidian (outside `docs/`).
  - **Toast component:** `.toast-container`/`.toast` in `style.css` + `showToast()`/`toasts[]` in `app.js` — scoped to batch/manual-assign feedback only; every other action still uses `alert()`/`confirm()` (not a full migration).
- **Verification:** API tests live in `tests/test_plant_ui_api.py`, `tests/test_photo_batch.py`, `tests/test_plant_photos_db.py`, `tests/test_claude_backend_vision.py`. Runs via `.venv/bin/pytest tests/`. Design doc: `docs/superpowers/specs/2026-07-12-batch-plant-photo-upload-design.md`.
